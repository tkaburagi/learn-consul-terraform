...
resource "hcp_aws_network_peering" "default" {
  peering_id      = "${data.hcp_hvn.server.hvn_id}-peering"
  hvn_id          = data.hcp_hvn.server.hvn_id
...
resource "hcp_hvn_route" "peering_route" {
  ...
  hvn_link         = data.hcp_hvn.server.self_link
  hvn_route_id     = "${data.hcp_hvn.server.hvn_id}-peering-route"
...
resource "aws_route" "peering" {
  ...
  destination_cidr_block    = data.hcp_hvn.server.cidr_block
