...
resource "aws_secretsmanager_secret_version" "bootstrap_token" {
  ...
  secret_string = var.consul_root_token_id
}
...
resource "aws_secretsmanager_secret_version" "gossip_key" {
  ...
  secret_string = jsondecode(base64decode(data.hcp_consul_cluster.example.consul_config_file))["encrypt"]
}
...
resource "aws_secretsmanager_secret_version" "consul_ca_cert" {
  ...
  secret_string = base64decode(data.hcp_consul_cluster.example.consul_ca_file)
}